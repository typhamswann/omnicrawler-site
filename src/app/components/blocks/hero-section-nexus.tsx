...
// ── Inside createDots ─────────────────────────────────────────
   ...
   const handleHeroFormSubmit = (e: FormEvent<HTMLFormElement>) => {
     e.preventDefault();
     if (!urlInput) return;
     
     // Store URL in sessionStorage to pass it to the scraper section
     sessionStorage.setItem('scraperUrl', urlInput);
     
     // Create and dispatch a custom event that the scraper section will listen for
     const event = new CustomEvent('triggerScraper', { detail: { url: urlInput } });
     window.dispatchEvent(event);
     
     // Scroll to the scraper section
    ...
    <div className="pt-[100px] relative bg-[#111111] text-gray-300 min-h-screen flex flex-col overflow-x-hidden">
        ...
        <main className="flex-grow flex flex-col items-center justify-center text-center px-4 pt-8 pb-8 relative z-10 min-h-[calc(100vh-90px)]">
            ...
            <motion.div
                ...
                className="mb-6"
            >
                <ShinyText text="Announcing our Agentic Scraper v1.0 🚀" className="bg-[#1a1a1a] border border-gray-700 text-[#60A4FA] px-4 py-1 rounded-full text-xs sm:text-sm font-medium cursor-pointer hover:border-[#60A4FA]/50 transition-colors" />
            </motion.div>
            ...
            >
A Universal Scraper that <span className="text-[#73abf5]">actually</span> works
</motion.h1>
            ...
            >
                AI just solved scraping. Turn <u>any</u> website into structured JSON or clean markdown.
            </motion.p>
            ...
            >
                <input
                    ...
                    className="flex-grow w-full sm:w-auto px-4 py-3 rounded-md bg-[#2a2a2a] border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#60A4FA] focus:border-transparent transition-all"
                />
                <motion.button
                    type="submit"
                    className="w-full sm:w-auto bg-[#60A4FA] text-[#111111] px-5 py-3 rounded-md text-sm font-semibold hover:bg-opacity-90 transition-colors duration-200 whitespace-nowrap shadow-sm hover:shadow-md flex-shrink-0"
                    whileHover={{ scale: 1.03, y: -1 }}
                    whileTap={{ scale: 0.97 }}
                    transition={{ type: "spring", stiffness: 400, damping: 15 }}
                >
